...
namespace QLM
{
    internal class MyLicenseValidator : LicenseValidator
    {
        ...
        {
            // Try to read keys from the current configured location, if not found, read from the ERegistry
            // and then save to EFileCommonData for next time.
                ...
                if (String.IsNullOrEmpty(activationKey))
                {
                    ...
                    try
                    {
                        ...
                        if (!String.IsNullOrEmpty(activationKey))
                        {
                            ...
                            // If we find keys in ERegistry, save to new location for next time.
                    ...
                    finally
                    {
                        this.license.StoreKeysLocation = saveLocation;
                        this.license.StoreKeysOptions = saveOptions;
                        // we will store the date we do migration so that we will not do migration again in the future.
